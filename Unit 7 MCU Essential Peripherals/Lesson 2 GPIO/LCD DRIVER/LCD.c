...
void LCD_INIT(void)
{
	_delay_ms(20);
	LCD_ISBUSY();
	DataDir_LCD_CTRL |= ((1<<EN_PIN)|(1<<RW_PIN)|(1<<RS_PIN)); // Set as output
	LCD_CTRL &= ~((1<<EN_PIN)|(1<<RW_PIN)|(1<<RS_PIN)); // Clear
...
void LCD_WRITE_CHAR(unsigned char character)
{
	#ifdef EIGHT_BIT_MODE
		LCD_ISBUSY();
		LCD_CTRL |= (1<<RS_PIN); // data mode
		LCD_PORT = (((character) << DATA_SHIFT));
		LCD_CTRL |= (1<<RS_PIN); //  data mode
		LCD_CTRL &= ~(1<<RW_PIN); // write mode
		LCD_KICK();
	#endif
